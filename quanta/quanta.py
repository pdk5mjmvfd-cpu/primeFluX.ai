...
class QuantaCompressor:
    ...
    def compute_quanta(self, capsule: Capsule | dict[str, Any]) -> float:
        ...
        compression_ratio = self.compression_ratio(raw_json, compressed)
        
        # Smooth QuantaCoin output for stability
        last = getattr(self, "_last_q", 1.0)
        smoothed = (last * 0.7) + (compression_ratio * 0.3)
        self._last_q = smoothed
        
        # Trust band to avoid extremes
        if smoothed < 0.5:
            smoothed = (smoothed + 0.5) / 2
        if smoothed > 3.0:
            smoothed = 3.0 - ((smoothed - 3.0) * 0.5)
